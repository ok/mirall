...
| s127 | `s127-folder-fault-strip.mjs` | **REGRESSION (FIX-PI12)** an unreadable file in the shared folder surfaces the fault strip ("couldn't be added — Permission denied", never the errno); it survives a restart, carries no verb for an owner, and clears on the next clean pass. |
