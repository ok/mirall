...
export default async function s39 ({ runDir, bootstrap }) {
  ...
  writeFileSync(path.join(ownDir, 'draft.mirall.part'), 'in-flight temp')
  ...
  try {
    ...
    await r.ok('the junk files are never published to the mirror', async () => {
      ...
      assert(!existsSync(path.join(mirrorDir, 'draft.mirall.part')), '*.mirall.part not shared')
    ...
    await r.ok('only the real file shows in B’s folder view', async () => {
      ...
      assert(!(await B.hasText('.mirall.part')), '*.mirall.part not listed')
    })
  } catch {}
