...
**Coverage at a glance:** the tables above are the curated subset — the *invite/join surface*, *input validation*, the *pure file/folder operation backbone* — and `ls test/unit` is the index; roughly half the files are source-scanning guards that pin structure (contract declarations, module boundaries, hook wiring) rather than behaviour. The data-layer I/O and multi-peer convergence for those operations stays at integration/flow by necessity; what this layer pins is the pure logic underneath them, where a cross-platform bug is cheapest to catch.
