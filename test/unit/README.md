...
- **Runner:** `npm run test:unit` (this layer + `test/invariants`) or `npm run test:node` (adds flow). Files match `test/unit/*.test.js`, run by `brittle-node` (Node, not Bare).
...
| File | Scenarios |
|------|-----------|
| `path-keys.test.js` | **rel ⇄ drive-key** (`relToDriveKey`/`driveKeyToSegments`): OS-sep → POSIX key; round-trip on **both** POSIX and Windows separators; top-level + nested. **Mount overlap** (`pathsOverlap`): equal/ancestor/descendant true, `/a/b` vs `/a/bc` sibling false, Windows separator. **Collision naming** (`splitFileName`/`nextFreeName`): `path.extname` parity (`a.tar.gz`, `LICENSE`, `.bashrc`, `file.`), `name (n)` walk, extension-less suffixing, in-flight partial treated as taken. **Deletion safety** (`shouldHonorDeletions`): four-quadrant truth table. **Ignore globs** (`shouldIgnore`/`DEFAULT_IGNORE`, the one matcher the main-process watcher and the reconcile share): exact basename anywhere, suffix-vs-prefix, a directory glob covering the tree beneath it + look-alikes, the gitignore vocabulary a share may carry (trailing slash, interior `*`, `?`, character class, leading-slash anchor, `!` negation), a non-string pattern, a path the matcher cannot judge, empty/missing patterns. **Mount name rules** (`systemRootViolation`/`isWindowsReservedName`): per-platform system roots, reserved device names with extension/case, name-prefix-sibling allowance. **Cloud-sync detection** (`cloudSyncHint`, gates a hard mount rejection): provider detection, ordinary folder → none. |

**Coverage at a glance:** the tables above are the curated subset — the *invite/join surface*, *input validation*, the *pure file/folder operation backbone* — and `ls test/unit` is the index. The source-scanning guards that pin structure rather than behaviour — contract declarations, module boundaries, hook wiring — now live in `test/invariants/` and have their own README. The data-layer I/O and multi-peer convergence for those operations stays at integration/flow by necessity; what this layer pins is the pure logic underneath them, where a cross-platform bug is cheapest to catch.

---

## How these became unit tests (the `path-keys` extraction)

The SEV1–3 gaps below were all **pure logic**, but they lived in modules that import `bare-path`/`bare-fs`/`bare-os`, which **do not load under plain Node** (`brittle-node`). That's the same reason `resolveDest`, `shouldHonorDeletions`, and `shouldIgnore` had only *integration* (Bare) tests despite being pure. To make them genuine fast Node unit tests **without duplicating logic**, the pure functions were lifted into `src/shared/folders/path-keys.js`, which imports nothing that needs Bare, and the heavy modules (`owned-folders`, `foreign-folders`, `files`, `share-listing`, `mount-validate`, `download-dest`) import from it — a single source of truth that also removed the 3× duplicated `split(sep).join('/')` conversion. The pre-existing integration tests still import via the original module paths and stay green, now doubling as wiring guards. Verified: unit + integration + flow all green; typecheck + lint clean.
...
| 6 | **`shouldIgnore`/`DEFAULT_IGNORE`** gitignore-spec matcher (incl. directory globs and negation) | `path-keys.js` | ignored junk in subfolders; `*.mirall.part` on copy |

### SEV-3 — robustness / advisory correctness ✅ implemented (except the noted #9)
| # | Pure logic | Now in | Guards |
|---|------------|--------|--------|
| 7 | **`systemRootViolation`/`isWindowsReservedName`** | `path-keys.js` | choosing where a shared folder lives (reject system/reserved paths) |
