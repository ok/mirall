...
// FolderView is keyed per share, and three separate pieces of state depend on that being true:
// useShareFiles folds each listing from emptyFold, useTreeExpansion seeds its expanded set once at
// mount from a per-share store, and FolderView's preFilterRef snapshots the expansion a filter is
...
test('nothing re-clears share state during render now that the key does it', (t) => {
  ...
  t.absent(/FolderView is reused/.test(src('hooks/useShareFiles.ts')),
