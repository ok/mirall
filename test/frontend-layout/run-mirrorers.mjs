// Mirrored-by sidebar widget test (LOCAL/dev-machine only — spawns a real Electron GUI process,
// like the agent-desktop frontend suite). Mounts the real <MirroredByWidget> in real Chromium and
...
// heading, and exposes an accessible name listing the mirrors and their states.
//
...
console.log(`\n${pass ? 'ok  ' : 'FAIL'} widget renders sync-state rings (no opacity), capped stack + overflow, heading + accessible name`)
