...
| `test:layout:case -- denyapproved` | `run-denyapproved.mjs` | `<SpaceView>` with a pending request a co-member already approved | Deny disables in flight; the already-approved outcome is a sticky polite status toast, never an alert; a deny with nothing open, and an approve on someone already in, are polite notices |
