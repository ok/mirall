...
- **Runner:** `npm run test:flow`, or `npm run test:node` (flow files run alongside `test/unit` and `test/raw`). `bare` must be on `PATH`.
- **Why not `brittle-node` directly:** `test/flow-runner.mjs` wraps brittle so a throwing test body (a timed-out `until()`, say) is recorded as a named `not ok` instead of rejecting an un-awaited promise and killing the process, a mid-run death exits non-zero, and every registered test is accounted for at exit — a truncated run fails. The last line of a run reads `# flow accounting: N/N registered test(s) executed`; anything less than N/N means the run was cut short. It wraps a brittle internal to do this, and asserts that internal's shape at startup: a `brittle` bump that moves the seam fails the run with a message naming the version, rather than quietly running unpatched.
...
| `mirror-local-edit.test.js` | **CRIT-7:** a mirror is owner-authoritative — if the user edits a file inside their own mirror, the walk the mirror's watcher requests keeps the edit as a conflicted copy and **restores the owner's version** — with no owner activity, no listing and no backstop. |
