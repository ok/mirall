...
// Alice's membership fold is held once she lists Bob's request, so Carol's approval is visible to
// Alice's admission read while Alice still lists the request — the stale banner a user actually
// clicks Deny on.
test('REGRESSION (FIX-395A: a deny racing a co-member approval leaves the request listed)', { timeout: scaled(300000) }, async (t) => {
  const bootstrap = await localTestnet(t)
  const hold = path.join(mkTmpDir(t), 'alice-fold-hold')
  const A = await peer(t, bootstrap, 'Alice', { memberFoldHold: hold })
  ...
  t.teardown(() => fs.rmSync(hold, { force: true }))
...
test('REGRESSION (FIX-395C: approving an approved member records the approval again)', { timeout: scaled(220000) }, async (t) => {
  ...
  t.alike(await A.request('space:approve-member', { spaceId: sid, publicKey: bKey }), { granted: false, outcome: 'already-approved' },
    'nothing new is granted, and the caller is told why')
  await t.execution(refreshed, 'a stale Approve row is told to refetch')
