...
test('one realistic session produces every expected kind, and nothing else', { timeout: scaled(300000) }, async (t) => {
  ...
  // B downloads a file OUT of A's folder share while still browsing it — the folder-engine
  // counterpart of the loose download above. Without this the session drives one producer of
  // transfer.completed, which is how the folder path stayed unrecorded through an "every declared
  // kind has a call site" test. It has to come before the mirror: a mirrored share refuses manual
  // downloads, since it syncs itself.
