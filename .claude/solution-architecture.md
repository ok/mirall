...
| Download folder not writable (`EACCES` / `EPERM` / `EROFS`, folder present) | `TRANSFER_PERMISSION` — **terminal**, never auto-resumed (the folder is still read-only on the next attempt). The row shows the error until the user retries; the same in-memory fallback as a checksum verdict applies |
