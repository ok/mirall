...
- **Still open (different bug):** a serve loop parked on stream backpressure is silent for up to `DRAIN_TIMEOUT_MS` (60 s), twice the downloader's window. A keep-alive cannot cross a backpressured stream, so this one needs its own answer.
