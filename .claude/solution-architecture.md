...
Recovery is **level-triggered** — reconnects and catalog changes re-drive the durable pending rows. No retry counters anywhere.
...
- **An oversized chunk** (tier-3 chunks reach 4 MB; a cap may be 64 KB/s) is released on a full bucket and the deficit repaid, or it could never be afforded at all.
...
| `folders/owned-folders.js` | Owner-side sync: `onFsEvent`, `initialPublishScan` / `previewInitialPublishScan`, periodic reconcile — all dispatched to the content backend. §7.2 |
