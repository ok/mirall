...
2. **Resolve the name** against the space's loose catalog (collision → free-name suffix; cap `MAX_LOOSE_FILES_PER_SPACE = 100`).
3. **Advertise a placeholder** catalog entry (no hash yet — peers render `preparing`), then stream the file **once**, computing the whole-file content hash and the chunk map in the same pass (`prepareForServe`, §7.7); finalize the entry and make the hash servable.
4. **Record + watch the source path** — kept for reveal-in-file-manager, watched so edits/moves are noticed.
...
2. **Initial scan** (`initialPublishScan`). A **diff-and-enqueue** producer: walks the tree **stat-only** honoring `ignore` globs (`DEFAULT_IGNORE` covers `.DS_Store`, `Thumbs.db`, `*.mirall.part`, `.git/**`, `node_modules/**`, …), diffs against the catalog by **size+mtime** — the git-index change signal, no reads for unchanged files — and enqueues one `publish` item per new/changed file and one `retire` item per catalog entry with no file behind it (never when the mount root itself is missing). It resolves once the share's items have settled, with `{ uploaded, deleted, totalOnDisk }`, which the worker emits as `event:owned-folder-scan-completed`. The content hash is published as entry metadata for mirrors to verify against; it does **not** drive owner-side change detection.
   **The publish queue** (`folders/publish-queue.js`, `publish-scheduler.js`, `publish-runner.js`, `work-item.js`). Every producer — mount, relocate, boot, the watcher, the catch-up and the periodic reconcile — only enqueues; one scheduler executes. A work item is keyed by **path** (`shareId\0relPath`), never by content hash: at most one live item per path, so a second request folds into a queued item or marks a running one dirty for exactly one rerun — a file can never be read twice concurrently. Queues are **per space**; a bounded runner (`publishConcurrency`, default 2) hands slots out round-robin across spaces, and while more than one space has work no space may hold every slot, so a large index in one space never stalls another. Within a space the order is configurable (`publishOrder`: `fifo` | `smallest-first` (default) | `largest-first`); an interactive item (a watcher event) outranks a bulk backfill, and a retire outranks a publish. Bulk items write through one catalog batch per space (few atomic heads for the consumer); interactive items write direct so a dropped-in file is visible at once. **Every executor re-derives its precondition from current state**: it re-resolves the mount (a relocate leaves enqueue-time paths stale), refuses to act when the root is missing (a vanished root makes chokidar emit one `unlink` per file), and a retire re-stats the file — absence from a minutes-old snapshot is a candidate, never a delete. `owned-folder:index-status` / `owned-folder:cancel-index` expose and cancel a share's queue; `event:owned-folder-index-progress` (decoration, coalesced) carries its progress.
3. **Live updates** (`onFsEvent`). `add`/`change` enqueue an interactive `publish`; `unlink` enqueues an interactive `retire`. The executor applies the guards (root must still exist, file confirmed gone — an editor's rename-over fires a raw unlink for a path that is immediately back). `echo-guard` drops events for paths the worker itself just wrote — no upload-of-our-own-download loop. A 2 s-debounced catch-up diff follows quiet periods (macOS fsevents coalescing drops adds); it enqueues only what is missing and leaves a file younger than 2 s with no catalog entry to the watcher's `awaitWriteFinish` rather than reading it mid-copy.
...
| `folders/owned-folders.js` | Owner-side producers: `onFsEvent`, `initialPublishScan` / `periodicReconcile` (diff → enqueue → drain), `previewInitialPublishScan`, the catch-up debounce, the per-space catalog batch, `getIndexStatus` / `cancelIndex`, teardown (`stopOwnedFolder`, `stopPublishingForSpace`, `stopAllPublishing`). §7.2 |
...
| `folders/publish-runner.js` | The executor: re-resolves the mount, root guard, re-stat, then `backend.publishAdd` / `publishDelete`. §7.2 |
