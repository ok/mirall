...
| `folders/publish-service.js` | The shared owner-side lane: the scheduler singleton, the per-space catalog batch (`catalogFor` / `settleCatalog` / `closeBatch`), the channel registry (`registerPublishChannel`), `whenPathIdle`, space/global stop. Owned folders and loose files both enqueue here. §7.2 |
| `folders/publish-runner.js` | The executor: dispatches on the item's `kind` to its channel — `resolve` (mount + root guard + path, or the loose source link), exact-name re-stat before a retire, then the channel's `publish` / `retire`. §7.2 |
