...
| Owner reconnects | The overlay reconnect hook fires `resumeLooseForOwner` / `resumeOverlayForOwner`, re-driving every pending row for that owner (skipping active and manually-paused rows, and every row whose `errorCode` is terminal per `isTerminalFault` — checksum, disk full, download folder unavailable, download folder not writable) |
...
| Download folder not writable (`EACCES` / `EPERM` / `EROFS`, and a probe write into the folder is refused too) | `TRANSFER_PERMISSION` — **terminal** while the folder stays read-only: a reconnect re-drives the row only once the folder takes a write again. The same errno with a writable folder (a file another program holds for a moment) is the retryable `DOWNLOAD_FAILED`. The same in-memory fallback as a checksum verdict applies |
