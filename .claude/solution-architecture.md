...
| `src/shared/network/space-topics.js` | `joinSpaceTopic` / `leaveSpaceTopic`, the new-space handshake to already-connected peers (Hyperswarm reuses sockets and fires no `'connection'`), and `cleanupSpaceDrives` — the leave-side peer eviction and its progress contract (§4.4, §6) |
...
| `src/shared/network/connectivity.js` | "Are we reachable": DHT / NAT verdict watchers, the two-stage canary probe, the liveness ping loop, the interface poll, `getSwarmStatus` assembly, the debounced `event:network-status` + audit hook, `reconnectAll`. Owns its reporting defaults (the `hyperdht` version read and the `swarm-diagnostics` instance) so status is answerable whether or not a `Swarm` was ever opened |
