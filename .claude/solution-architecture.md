...
| `src/worker/ipc/settings.js` | `registerSettings(ipc, deps)` — storage info, the bandwidth limits (read per call, so they reach in-flight transfers), the global download folder with the same mount-overlap rejection a per-space folder gets, the download-root probe, the feature flags and the live verbose toggle (§5) |
