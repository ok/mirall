...
> **Scope.** Authoritative for process model, data model, networking, IPC, update mechanics, security model. Only what the code cannot tell a reader: cross-process ordering, persisted and wire formats, trust boundaries, and the reasons behind decisions. File layout is the tree itself (each file's header states its job); UI structure, dependencies, packaging and testing live in `design.md`, `dependency-updates.md`, `build-process.md` and `testing.md`. Section numbers are stable, so retired sections leave gaps.
...
**Stack.** Versions: `package.json`; pins and their reasons: `dependency-updates.md`. Placement rules: `chokidar` runs in Electron main only (Bare has no recursive watch); `bare-*` modules are worker-only; `purgeCoreDk()` uses `hypercore-storage` internals (transitive dep) for raw RocksDB range deletes.
