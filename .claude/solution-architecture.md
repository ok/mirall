...
| `src/shared/contract/deny-outcome.js` | `DENY_OUTCOME` — what `space:deny-member` did: `denied`, `already-approved` (approval cannot be revoked and key rotation is not implemented, so the renderer says so), or `not-applicable` (no open request) |
