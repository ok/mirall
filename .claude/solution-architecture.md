...
| `src/shared/folders/mirror-loop.js` | The per-mount loop, with no knowledge of catalogs, hashes or mounts: one interval per key, at most one pass in flight, a dirty flag, a cancellation generation, the liveness heartbeat. `invalidate` cancels the passes in flight without disarming the cadence: a pause, relocate or unmount calls it before its record write, and every pass write declines inside the serialized write once its generation is stale |
