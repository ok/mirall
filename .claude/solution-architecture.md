...
| File | Purpose |
|---|---|
| `src/shared/folders/foreign-folders.js` | The mirror's mount lifecycle (§7.3): loop wiring, level triggers, orphan unmount, health / restart, `ForeignMirrors`, and the unmount / relocate / enable verbs |
...
| `src/shared/folders/mirror-pass.js` | One walk of a mirrored share's catalog: for every entry, already right / fetch / withhold, plus the reachability gate the walk and the deletion guard share. Restartable by design — everything here is derived fresh from the catalog each tick, which is why it is separate from the fetch that is not (§7.3) |
