...
| `src/shared/contract/deny-outcome.js` | `DENY_OUTCOME` — what `space:deny-member` did: `denied`, `already-approved` (approval cannot be taken back before the key rotates, so the renderer says so), or `not-applicable` |
