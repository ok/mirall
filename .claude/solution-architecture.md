...
| `src/worker/ipc/foreign-folders.js` | `registerForeignFolders(ipc, deps)` — mirror mounts of a share someone else owns: validate, mount (which arms the poll loop whatever the initial scan does), get, enable, relocate and unmount. Removal runs under an intent because the mount record and the mirror record live in two stores (§7.6) |
