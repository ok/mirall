...
log drains it on close. State that means "we recorded this" follows its `true` (a `false` admitted
nothing, so a retry cannot duplicate). `audit-writes-not-silenced.test.js` flags a silenced `record`.
