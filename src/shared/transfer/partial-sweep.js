...
import { PARTIAL_SUFFIX as OVERLAY_PARTIAL_SUFFIX, journalNameFor } from './backends/overlay/vendor/transfer.js'
import { getJournalDir } from './backends/overlay/overlay-instance.js'
import { createLogger } from '../core/logger.js'

const log = createLogger('partial-sweep')

// Sweep orphaned `.overlay-partial` files. The overlay/loose engine leaves one at
// `<finalPath>.overlay-partial` while a transfer is in flight. A partial is orphaned
...
export async function cleanupOrphanedOverlayPartials (downloadsDir, mountDirs = []) {
  const referenced = new Set()
  for (const p of await listPending()) {
    if (p.finalPath) referenced.add(p.finalPath + OVERLAY_PARTIAL_SUFFIX)
  }
  let journalDir = null
  ...
  const isResumable = (full) => {
    ...
    const target = full.slice(0, -OVERLAY_PARTIAL_SUFFIX.length)
  ...
  const sweepOne = async (full) => {
    ...
    try {
      ...
      log.info('swept orphaned overlay partial:', full)
    } catch (err) {
      if (err.code !== 'ENOENT') log.warn('overlay partial sweep unlink failed:', full, err.message)
    }
  }
  ...
  for (const name of names) {
    if (name.endsWith(OVERLAY_PARTIAL_SUFFIX)) await sweepOne(path.join(downloadsDir, name))
  }
...
// Best-effort recursive collect of `.overlay-partial` files under `dir`. Metadata-only
...
async function collectPartials (dir, out = []) {
  ...
  for (const name of names) {
    ...
    else if (name.endsWith(OVERLAY_PARTIAL_SUFFIX)) out.push(full)
  }
  return out
}
