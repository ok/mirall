...
export function pathFromMount(mountPath, relPath) {
  if (relKeyEscapes(relPath)) {
    throw new AppError(CODES.EPATH, `file path rejected — unsafe segment escapes the share folder: ${relPath}`)
  }
  const abs = path.join(mountPath, ...relPath.split('/'))
  const rel = path.relative(mountPath, abs)
  if (rel === '' || rel === '..' || rel.startsWith('..' + path.sep) || path.isAbsolute(rel)) {
    throw new AppError(CODES.EPATH, `file path rejected — resolves outside the share folder: ${relPath}`)
  }
  return abs
}
