...
// and a late completion re-writes purged meta rows. Per-id best-effort: cancel throws when the row
// cannot be cleared, and the leave's own clearPendingForSpace purges the rows a beat later — one
// failed discard must not abort the leave.
...
// path; `buildJob(slot, state, transferId)` produces the supersede job (null when it cannot be rebuilt).
